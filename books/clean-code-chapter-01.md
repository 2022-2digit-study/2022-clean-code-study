...
<br>
**프로그래밍 언어를 컴파일하고 실행시키는 주체는 컴퓨터**이나, 프로그램은 단 한번 실행하고 멈추는 것이 아니라, **지속적으로 유지보수하고 발전해나가는 존재**이다.<br>
따라서 프로그래밍 언어의 본질은 2번에 있다.
...
클린코드를 정의하는 여러 문서들이 있다. 특히 여러 [블로그](https://hypers84.tistory.com/2460)에도 클린코드의 5가지 원칙을 말하면서, 클린코드에 대한 설명을 자주 찾아볼 수
있다.<br>
> 그러나 저자는 `"클린코드는 이런거야!"` 하고 정의를 내리기 보다, 책을 통해 스스로 **좋은 코드**와 **나쁜 코드**를 구분하는 것을 권장한다.

### + 클린코드 X 코드 포매팅

가끔 코드 포매팅을 잘하는 것이 곧 클린코드라고 말하는 것을 볼 수 있다.<br>
<br>
그러나 [PEP-8](https://peps.python.org/pep-0008/)과 같은 지침에 따라 **코드를 포매팅 하고 구조화하는 것은 클린코드의 최소요건**이며 코드 포매팅이 클린코드라는 것은 틀린 명제이다.
...
분명 PEP-8과 같은 코딩 가이드를 준수하는 것은 소프트웨어의 일관성을 유지하는 것에 도움을 준다.<br>
<br>
특히 일관성 있는 코드는 오류를 감지하기 쉽게하며,<br>
신속하게 패턴을 파악하여 개발자의 이해를 높일 수 있다.
...
<br>
<br>
그러나 가이드를 지키지 않아도 코드는 얼마든지 깔끔하게 적을 수 있다.
...
중요한 것은 이렇게 가이드를 지키지 않고 열을 옮겼을 뿐인데도 얼마든지 깔끔하게 코드를 적을 수 있다는 것이 아니다. 오히려 이런 포매팅을 하는 자동화 도구를 만든다면 이런 가이드를 준수하도록 만드는 것이 매우 어려울 것이다.<br>
<br>
따라서 코드 포매팅은 코드를 깔끔하게 보기 위한 최소요건이지 이 자체가 클린코드라는 명제는 따라서 **틀린 명제**가 된다.
...
`1번의 경우`는 오래 방치되어 곳곳에 균열이 생겼다. 운전자는 균열을 피해서 돌아가야 하기 때문에 시간이 지체될 것이고, 예정한 시간보다 늦어질 수 있을 것이다. <br>
<br>
`2번의 경우`는 그냥 도로를 달리면 된다. 이렇게 관리가 잘 된 도로라면 오히려 예정된 시간보다 빨리 도착할 수 있다.<br>
<br>
이처럼 _클린 코드를 쓴다._ 라는 것은 다음과 같은 기대효과를 가질 수 있다.
...
굳이 설명을 하지 않아도 되는 코드가 좋은 코드겠지만, 자신이 썼던 코드를 이해할 때나, 다른 사람이 썼던 코드를 이해해야할 때 주석이 있으면 조금 더 편리한 것이 사실이다.<br>
<br>
**코드를 설명하는 방법**
...
### 2. 함수의 이름과 파라미터의 이름
함수의 이름과 파라미터의 이름을 충분히 설명적이라면 매우 좋다. 하지만 이상과 현실은 다름을 명시하자.<br>
<br>
예)
```python
...
`docstring`은 각 함수나 클래스에 추가할 수 있는 설명 **문서**이다.
...
### 호출
파이썬을 실행을 통해 `docstring`에 적용되어 있는 값을 알고 싶다면 `__doc__`를 통해 호출할 수 있다.
...
사용법은 충분히 알아보았으니 왜 `docstring`을 권장하는지 알아보자.<br>
<br>
가장 큰 이유는 **파이썬이 동적 타이핑을 하기 때문이다.** 파이썬은 파라미터 타입이나 반환 타입 체크를 강요하지 않는다. 다른 언어를 배워본 적이 있다면, **파라미터나, 리턴 타입을 통해서 어떤 데이터가 들어오고 나가는 지 추론할 수 있다**는 것을 알 수 있을 것이다.

> 하지만 우리의 파이썬은 그렇지 않다.

단순히 Python 코드와 Java 코드를 비교해보자.
```java
...
위의 자바 코드에서는 유저 객체를 매개변수로 받아 문자열 데이터 타입인 `String`으로 반환하는 것을 직관적으로 알 수 있다.<br>
<br>
하지만 아래 파이썬 코드는 조금 다르다. `get` 이라는 키워드를 보아 뭔가 유저의 이름을 가져오는 것을 알 수 있다.<br>
하지만 그 유저의 이름이 어떤 유저의 이름만을 위해 만들어 진 객체인지, 아니면 단순 문자열인지는 **알 수 없다.**<br> 
> 일례로 사실 아래 코드의 어떤 모듈에는 이런 객체들이 선언되어있을 지도 모른다.<br>
```python
class UserName:
...
<br>
물론, 함수명이나 파라미터 명에서 충분히 설명되어 있다면 이를 보완할 수 있다. 그게 아니더라도 이는 코드를 보면 쉽게 알 수 있을 수도 있다. <br>
다만 게으른 개발자에게 있어 이는 너무나도 고독한 처사일지 모른다. 주석을 남겼더라면, 더 좋은 방법일 수 있다.<br>
하지만 모듈을 하나하나 찾아서 들어가서 주석을 찾아보는 것은 여전히 번거롭다. <br>
<br>
이런 번거로운 개발자에게는 그것보다는 파이썬의 대화형 인터프리터를 통해 바로 실행하여 쉽게 알 수 있는, 아래와 같은 `docstring` 이 답이 될 수 있다.
...
아니다. 위에 필자가 적은 코드 예시와 가이드들을 살펴보아도, `docstring`이 효과적으로 동작하기 위해서는 보다 상세한 설명을 기입해야 함을 알 수 있다.<br>
<br>
다만 그러면 그럴 수록, 필연적으로 코드보다 `docstring`이 크기가 커질 것이다.<br>
<br>
어떤 좋은 가이드를 봐도, 어떤 블로그를 보아도 이상적인 설명의 해답은 **설명 없이 코드만 보아도 직관적으로 이해가 되는 코드**이다. 
...
필자는 Docstring에 대해 간략하게 알아보지만 여러 방법으로, 여러 개발사에서 docstring가이드에 대한 부분을 명시해 놓았기 때문에, 이는 개발자가, 프로젝트 팀원끼리 자신의 프로젝트에 맞는 docstring 가이드를 선택하는 것이 가장 좋은 방법일 것이라 생각한다.
...
사실 자바라는 언어를 접했다면 '어? 익숙하네?' 라고 생각할 수 있다.<br>
하지만 자바에서의 어노테이션과 파이썬의 어노테이션은 많이 다르다.

### 어노테이션이 뭐야?

파이썬의 어노테이션은 확실히 설명을 위해서 만들어져 있다. 바로 우리가 원하던 **"타입"** 에 말이다. <br>
**어노테이션은 힌트를 활성화시켜주는 역할을 수행**한다.<br>
<br>
직관적으로 알아보기 위해 바로 코드를 보자
...
아까 작성한 코드에 타입 힌트(Type Hint)를 명시하였다. 이로써 아까 적어둔 Java 코드와 비슷해졌다. 이처럼 어노테이션을 명시한다면 어떤 타입을 파라미터로 받는지, 어떤 타입을 리턴하는지 쉽게 알 수 있다. 개발자가 위의 코드를 본다면 `User` 인스턴스를 파라미터로 받아서 `UserName` 인스턴스로 반환한다는 것을 알 수 있을 것이다.<br>
<br>

### 강제...로...?

이렇게 말했으니 이렇게 질문할 수 있다. _"그럼 타입힌트를 명시하면 실행시키면 타입과 다를 때 에러가 나는거야?"_ <br>
<br>
답을 말하자면 **아닙니다.**

`PEP484`에서는 이에 대한 답을 살펴볼 수 있다.

> _"파이썬은 여전히 동적인 타입의 언어로 남을 것이다. 타입 힌트를 필수로 하자거나 심지어 관습으로 하자는 것은 전혀 아니다."_

### 확인

어노테이션은 인터프리터나 코드에서 타입 힌트를 명시한 함수에 `some_func.__annotations__`을 통해 확인할 수 있다.
...
이렇게 쉽게 어노테이션으로 타입과 클래스 위치까지 확인할 수 있다면, `docstring`의 많은 부분을 대체할 수 있을 것이다.
```
"""
    Description: 
        유저의 객체를 통해 유저의 이름을 알아내는 함수 입니다.
...
```python
def get_user_name(user: User) -> UserName:
    """User 객체를 통해 유저의 이름을 알아내는 함수 입니다."""
    ...
    return user_name
...
많은 `Lint`들을 활용하면 타입 힌트를 명시할 때 타입을 검사하고 타입 힌팅에 명시한 클래스와 호환되지 않는 타입의 경우 경고를 통해 개발자에게 지침을 줄 수도 있다.<br>
<br>
...
> 많이 사용하는 JetBrains계열의 IDE PyCharm에서는 명시한 타입 힌트를 어기는 데이터 타입을 발견했을 때 다음처럼 노란줄을 통해 알려주기도 한다.

<br><br>

**+ 타입힌트에 명시한 파라미터를 이용할 때 해당 객체의 메소드를 사용할 수 있다.**
